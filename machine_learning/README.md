...
To showcase the concept, we use the credit card dataset from Kaggle (download [here](https://www.kaggle.com/mlg-ulb/creditcardfraud)). Note, the dataset is highly imbalanced (0.172% of signal) and we do not rebalance the dataset. After re-scaling the features and training, we get a AUC-ROC of  91% on the test set. The main libraries are `scikit-learn` and `tensorflow` and we use as template the autoencoder code from [tensorflow.org](https://www.tensorflow.org/tutorials/generative/autoencoder) and adjust it to our use case.   
